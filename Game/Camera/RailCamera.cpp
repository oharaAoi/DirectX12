...
void RailCamera::Update() {
	...
	cameraObj_->GetTransform()->SetTranslation(transform_.translate);
	cameraObj_->Update();
	BaseCamera::Update();
}
...
void RailCamera::RailMove() {
	...
	transform_.translate = controlPoints_[eyeIndex_];
	// forwardの座標を求める
	Vector3 forwardPos = controlPoints_[forwardIndex_];
