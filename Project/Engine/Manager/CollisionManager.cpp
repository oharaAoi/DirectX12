...
void CollisionManager::CheckAllCollision() {
	...
	std::list<MeshCollider*>::iterator meshIterA = meshColliders_.begin();
	for (; meshIterA != meshColliders_.end(); ++meshIterA) {
		MeshCollider* colliderA = *meshIterA;

		// イテレータBはイテレータAの次の要素から回す
		std::list<MeshCollider*>::iterator meshIterB = meshIterA;
		meshIterB++;

		for (; meshIterB != meshColliders_.end(); ++meshIterB) {
			MeshCollider* colliderB = *meshIterB;
			// ペアの当たり判定
			CheckCollisionPair(colliderA, colliderB);
		}
	}
...
bool CollisionManager::CheckCollisonObb(MeshCollider* colliderA, MeshCollider* colliderB) {
	// 分離軸の組み合わせを取得
	std::vector<Vector3> crossSeparatingAxises;
	for (uint8_t obb1Index = 0; obb1Index < 3; obb1Index++) {
		for (uint8_t obb2Index = 0; obb2Index < 3; obb2Index++) {
